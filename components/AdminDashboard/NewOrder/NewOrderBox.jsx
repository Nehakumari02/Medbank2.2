...
const NewOrderBox = () => {
  ...
  // Function to fetch CSV data and extract sample names
  // async function fetchSampleNames(url) {
  //   try {
  ...
  //           console.log(cells)
  //           if (cells.length > 0) {
  //               sampleNames.push(cells[1]); // Extracting sample name from the first column
  //           }
  //       }

  //       // console.log(sampleNames); // Output the sample names
  //       return sampleNames;
    ...
    try {
        ...
        for (let i = 8; i < rows.length; i++) {
            ...
            if (cells.length > 1 && cells[1].trim() !== "") {
                sampleNames.push(cells[1].trim()); // Extracting sample name from the second column
            }
        }
    ...
    try {
      ...
      uploadRequest.onload = async () => {
        if (uploadRequest.status === 200) {
          ...
          setQualityCheckReportLink(fileUrl);
          setQualityCheckStatus("isAdminCompleted");
      ...
      <div className='text-[#333333] mb-[14px] flex flex-col justify-between h-full'>
            ...
            <div ref={orderPopUpBoxRef}>
              {activePopup === 'requestSheet' && (
                <div className='p-[10px] w-[298px] h-[197px] md:h-[334px] md:w-[760px] md:p-[120px] flex flex-col gap-[24px] items-center justify-center bg-white border-[1px] border-[#D9D9D9] rounded-[10px] shadow-[0px_8px_13px_-3px_rgba(0,_0,_0,_0.07)]'>
                  ...
                  <div className='flex items-center justify-center gap-[12px]'>
                    <button onClick={() => handleDownload(requestSheetLink.split("?")[0], `RequestSheet.csv`)} disabled={disabled} className={`${disabled ? "opacity-75" : ""}`}>
              ...
              {activePopup === 'invoice' && (
                <div className="bg-white rounded-md shadow-lg py-[12px] md:py-[26px] md:px-[12px] w-[90vw] lg:w-[1199px] mx-5 px-4 md:mx-auto my-10 font-DM-Sans md:min-h-[576px]">
                  ...
                  <div className='border border-dashed'></div>
                  <div className='border border-dashed pt-[20px]'></div>
                  <div className="w-full overflow-y-auto max-h-[50vh] mb-[10px] pb-[10px] pl-[10px] overflow-x-scroll custom-scrollbar">
                    <table className="w-full min-w-[768px]">
                      <thead className='sticky top-0 bg-white'>
                        <tr className="text-left font-medium text-sm">
        ...
        <div>
          ...
          <div className='flex items-center justify-center md:justify-start gap-x-[6px] gap-y-[6px]  md:gap-x-[32px] md:gap-y-[8px] flex-wrap'>
            <button onClick={handleOrderCreation} disabled={!(requestSheetStatus == "inAdminProgress" || requestSheetStatus == "isUserCompleted" || requestSheetStatus == "isCompleted")} className={`h-[44px] w-[113px] md:h-[64px] md:w-[184px] p-[4px] md:p-[8px] rounded-[4px] md:rounded-[6px] ${requestSheetStatus == "isPending" || requestSheetStatus == "inUserProgress" ? "text-[#333333]" : "text-white"} ${requestSheetStatus == "isPending" || requestSheetStatus == "inUserProgress" ? "bg-[#E2E8F0]" : requestSheetStatus == "inAdminProgress" || requestSheetStatus == "isUserCompleted" ? "bg-[#FF914D]" : "bg-[#5CE1E6]"} font-DM-Sans font-medium text-[8px] md:text-[14px] leading-[24px] text-center`}>{t("buttons.requestSheet")}</button>
            <button onClick={handleCostEstimateClick} disabled={!(costEstimateStatus == "inAdminProgress" || costEstimateStatus == "isUserCompleted" || costEstimateStatus == "isCompleted")} className={`h-[44px] w-[113px] md:h-[64px] md:w-[184px] p-[4px] md:p-[8px] rounded-[4px] md:rounded-[6px] ${costEstimateStatus == "isPending" || costEstimateStatus == "inUserProgress" ? "text-[#333333]" : "text-white"} ${costEstimateStatus == "isPending" || costEstimateStatus == "inUserProgress" ? "bg-[#E2E8F0]" : costEstimateStatus == "inAdminProgress" || costEstimateStatus == "isUserCompleted" ? "bg-[#FF914D]" : costEstimateStatus == "isAdminCompleted" ? "bg-[#79747E]" : "bg-[#5CE1E6]"} font-DM-Sans font-medium text-[8px] md:text-[14px] leading-[24px] text-center`}>{t("buttons.costEstimation")}</button>
            ...
            <button onClick={handleAnalysisSpecificationClick} disabled={!(analysisSpecificationStatus == "inAdminProgress" || analysisSpecificationStatus == "isUserCompleted")} className={`h-[44px] w-[113px] md:h-[64px] md:w-[184px] p-[4px] md:p-[8px] rounded-[4px] md:rounded-[6px] ${analysisSpecificationStatus == "isPending" || analysisSpecificationStatus == "inUserProgress" ? "text-[#333333]" : "text-white"} ${analysisSpecificationStatus == "isPending" || analysisSpecificationStatus == "inUserProgress" ? "bg-[#E2E8F0]" : analysisSpecificationStatus == "inAdminProgress" || analysisSpecificationStatus == "isUserCompleted" ? "bg-[#FF914D]" : analysisSpecificationStatus == "isAdminCompleted" ? "bg-[#79747E]" : "bg-[#5CE1E6]"} font-DM-Sans font-medium text-[8px] md:text-[14px] leading-[24px] text-center`}>{t("buttons.analysisSpecification")}</button>
            <button onClick={handleInvoiceClick} disabled={!(invoiceStatus == "inAdminProgress" || invoiceStatus == "isUserCompleted" || invoiceStatus == "isCompleted")} className={`h-[44px] w-[113px] md:h-[64px] md:w-[184px] p-[4px] md:p-[8px] rounded-[4px] md:rounded-[6px] ${invoiceStatus == "isPending" || invoiceStatus == "inUserProgress" ? "text-[#333333]" : "text-white"} ${invoiceStatus == "isPending" || invoiceStatus == "inUserProgress" ? "bg-[#E2E8F0]" : invoiceStatus == "inAdminProgress" || invoiceStatus == "isUserCompleted" ? "bg-[#FF914D]" : invoiceStatus == "isAdminCompleted" ? "bg-[#79747E]" : "bg-[#5CE1E6]"} font-DM-Sans font-medium text-[8px] md:text-[14px] leading-[24px] text-center`}>{t("buttons.invoice")}</button>
